...
const activityExamplesRu = [
  {
    name: 'Дебатный клуб',
    category: 'Дебаты / Ораторское искусство',
    weak: 'Участник дебатной команды 3 года.',
    strong:
      'Капитан основной команды; вывел команду в полуфинал штата. Тренировал 8 новичков, 5 прошли на региональный этап. Организовал школьный публичный форум по политике образования.',
    why: 'Сильная версия подкреплена цифрами, показывает рост лидерства и инициативу, выходящую за рамки простого участия.',
  },
  {
    name: 'Репетиторство',
    category: 'Академический',
    weak: 'Помогал ученикам после школы.',
    strong:
      'Основал бесплатную программу репетиторства для 40+ учеников в неделю по математике и физике. Привлёк и обучил 12 волонтёров. Средний рост оценок: на 1.5 балла.',
    why: 'Цифры рассказывают историю. Сильная версия показывает, что ты создал что-то, масштабировал и измерил результат.',
  },
  {
    name: 'Подработка',
    category: 'Работа (оплачиваемая)',
    weak: 'Работал в магазине.',
    strong:
      'Старший смены, управлял командой из 6 человек. Оптимизировал систему учёта товаров, сократив потери на 15%. Работал 20 ч/нед, сохраняя GPA 3.9.',
    why: 'Даже оплачиваемая работа может продемонстрировать лидерство и решение проблем. Покажи уровень ответственности, улучшения и баланс дел.',
  },
  {
    name: 'Оркестр',
    category: 'Музыка',
    weak: 'Играл на скрипке в оркестре.',
    strong:
      'Первая скрипка, All-State оркестр 2024-25. 12 концертов в год, включая Карнеги-Холл. Наставлял 3 младших учеников при переходе с методики Судзуки.',
    why: 'Конкретные достижения, площадки выступлений и наставничество превращают обычную активность в убедительную историю мастерства и помощи другим.',
  },
  {
    name: 'Волонтёрство в продуктовом банке',
    category: 'Общественная деятельность',
    weak: 'Волонтёрил в продуктовом банке.',
    strong:
      'Координатор логистики городского продуктового банка; организовывал еженедельные раздачи для 200+ семей. Создал базу данных в Excel для учёта пожертвований, сократив порчу на 30%.',
    why: 'Должность и конкретный вклад показывают ответственность. Технические навыки в волонтёрской работе демонстрируют находчивость.',
  },
]

const honorExamplesRu = [
  { name: 'National Merit Semifinalist', level: 'Национальный', description: 'Набрал баллы в топ-1% по PSAT по стране (Selection Index: 223). Прошёл в раунд финалистов.' },
  { name: 'AP Scholar with Distinction', level: 'Национальный', description: 'Получил баллы 3 и выше на 5+ экзаменах AP со средним баллом не менее 3.5.' },
  { name: 'Science Olympiad — 2-е место по физике', level: 'Штат', description: 'Занял 2-е место в Circuit Lab на олимпиаде штата среди 84 команд.' },
  { name: 'Ученик месяца — октябрь', level: 'Школьный', description: 'Выбран преподавателями за академические успехи и лидерство. Один из 10 учеников, отмеченных за год.' },
  { name: 'Опубликованная научная статья', level: 'Национальный', description: 'Соавтор статьи в Journal of Young Investigators об устойчивости к антибиотикам в местных водоёмах.' },
]

const notHonorsRu = [
  { item: 'Список отличников / Dean\'s List', reason: 'Укажи это в GPA или академическом профиле.' },
  { item: 'Сертификаты участия', reason: 'Посещение мероприятия — это активность, а не награда. Укажи в разделе «Активности».' },
  { item: 'Членство в клубе', reason: 'Членство — это участие. Указывай только если занимал лидерскую роль или прошёл конкурсный отбор.' },
  { item: 'Завершение курса или программы', reason: 'Завершение ожидаемо. Указывай только при отличии (топ-10%, лучшая оценка и т.д.).' },
  { item: 'Состав спортивной команды', reason: 'Попадание в команду — это активность. MVP, All-Conference или звание капитана — награды.' },
]

const researchExamplesRu = [
  {
    title: 'Устойчивость к антибиотикам в городских водоёмах',
    type: 'Научное исследование',
    status: 'Опубликовано',
    description: 'Исследовал распространённость антибиотикоустойчивых бактерий в трёх городских ручьях с использованием культуральных и ПЦР-методов в течение 6 месяцев.',
    contribution: 'Разработал протокол отбора проб, собрал 120+ образцов воды, выполнил экстракцию ДНК и ПЦР-анализ, написал разделы «Методы» и «Результаты» опубликованной статьи.',
    skills: ['ПЦР', 'Гель-электрофорез', 'Статистический анализ (R)', 'Научное письмо', 'Лабораторная безопасность'],
    mentor: 'Др. Сара Чен, кафедра биологии Мичиганского университета',
  },
  {
    title: 'Приложение для навигации в транспорте для слабовидящих',
    type: 'Самостоятельный проект',
    status: 'Завершён',
    description: 'Разработал прототип мобильного приложения с аудионавигацией в реальном времени для общественного транспорта с использованием GPS и text-to-speech API.',
    contribution: 'Единственный разработчик. Провёл интервью с 15 слабовидящими пассажирами, создал прототип на React Native, прошёл 3 раунда пользовательского тестирования.',
    skills: ['React Native', 'JavaScript', 'REST API', 'Пользовательские исследования', 'Доступность (WCAG)'],
    mentor: 'Самостоятельная работа; обратная связь от школьного учителя информатики',
  },
  {
    title: 'Влияние микропластика на размножение дафний',
    type: 'Конкурсное исследование',
    status: 'Завершён',
    description: 'Спроектировал и провёл контролируемый эксперимент по влиянию различных концентраций полиэтиленового микропластика на размножение Daphnia magna в течение 21 дня.',
    contribution: 'Разработал гипотезу и дизайн эксперимента, вёл 24 культуры, ежедневно подсчитывал потомство, выполнил ANOVA-анализ, создал постер и устную презентацию.',
    skills: ['Дизайн экспериментов', 'Микроскопия', 'Анализ данных (Excel)', 'Научная презентация', 'Уход за животными'],
    mentor: 'Мисс Ривера, учитель AP Biology; наставник на региональной ярмарке науки',
  },
]

const essayApproachesRu = {
  personal: {
    title: 'Личное эссе',
    topics: [
      { idea: 'Провалы с хлебом на закваске', angle: 'Используй серию неудачных попыток как призму для размышлений о терпении, точности и умении учиться на ошибках. Свяжи с подходом к учёбе.' },
      { idea: 'Перевод для бабушки', angle: 'Исследуй, как роль моста между языками сформировала твою идентичность, ответственность и понимание нюансов. Опирайся на один конкретный, яркий момент.' },
      { idea: 'Заметки на полях', angle: 'Опиши привычку заполнять поля книг вопросами и аргументами. Покажи, как активное чтение стало активным мышлением и сформировало интеллектуальную идентичность.' },
    ],
    dos: [
      'Начни с конкретного момента или образа, а не с общего утверждения',
      'Покажи уязвимость и искреннюю рефлексию',
      'Свяжи историю с тем, кто ты сейчас, а не только с тем, что произошло',
      'Используй свой голос и естественный язык',
      'Раскрой то, что не видно в остальной части заявки',
    ],
    donts: [
      'Пересказывать резюме или перечислять достижения',
      'Писать о теме только потому, что она звучит впечатляюще',
      'Использовать клише типа «Я понял ценность трудолюбия»',
      'Фокусировать всё эссе на другом человеке (бабушке, тренере и т.д.)',
      'Пытаться шутить, если юмор — не твой естественный стиль',
    ],
  },
  whyUs: {
    title: 'Эссе «Почему именно этот вуз»',
    structure: [
      { part: 'Зацепка', detail: 'Начни с конкретной детали о вузе, которая тебя по-настоящему вдохновляет.' },
      { part: 'Академическое соответствие', detail: 'Назови конкретные программы, профессоров, курсы или исследовательские возможности. Объясни, что бы ты с ними делал.' },
      { part: 'Культура кампуса', detail: 'Упомяни традицию, клуб или ценность, которая совпадает с тем, кто ты. Будь конкретен.' },
      { part: 'Твой вклад', detail: 'Объясни, что ты привнесёшь в это сообщество. Свяжи прошлый опыт с планами на кампусе.' },
    ],
    dos: [
      'Изучи глубоко: упоминай детали, которые можно найти только на сайте вуза, при визите или в разговоре со студентами',
      'Объясни, ПОЧЕМУ каждая деталь важна лично для тебя',
      'Покажи, что у тебя есть план на время обучения',
      'Свяжи свои интересы с уникальными предложениями вуза',
    ],
    donts: [
      'Упоминать только рейтинг, престиж, расположение или погоду',
      'Писать настолько обобщённо, что текст подойдёт любому вузу',
      'Просто перечислять программы без объяснения твоей связи с ними',
      'Копировать маркетинговый язык вуза',
    ],
  },
  community: {
    title: 'Эссе о сообществе',
    communities: [
      'Твоя семья или домохозяйство',
      'Культурная или религиозная группа',
      'Онлайн-сообщество или форум',
      'Команда, клуб или ансамбль',
      'Твой район или город',
      'Группа по общей идентичности',
      'Рабочий коллектив или волонтёрская организация',
    ],
    dos: [
      'Определи, что сообщество значит лично для тебя',
      'Покажи свою конкретную роль и вклад',
      'Опиши момент напряжения или роста внутри сообщества',
      'Расскажи, как сообщество сформировало твои ценности',
    ],
    donts: [
      'Просто описывать сообщество, не показывая своё место в нём',
      'Выбирать сообщество, с которым у тебя нет настоящей связи',
      'Фокусироваться только на том, что сообщество дало тебе, не показывая, что ты дал взамен',
      'Быть расплывчатым в том, о каком сообществе ты пишешь',
    ],
  },
  challenge: {
    title: 'Эссе о трудностях / неудачах',
    framework: [
      { step: 'Ситуация', detail: 'Кратко опиши, что произошло. Будь честен, но не зацикливайся на негативе.' },
      { step: 'Твоя реакция', detail: 'Что ты конкретно СДЕЛАЛ? Покажи инициативу и принятие решений, а не только эмоции.' },
      { step: 'Рост', detail: 'Как это изменило твоё мышление, поведение или цели? Будь конкретен и честен.' },
      { step: 'Применение', detail: 'Покажи, как этот рост проявляется в твоей жизни сейчас. Свяжи прошлое с настоящим.' },
    ],
    dos: [
      'Выбери настоящую трудность, а не скрытое хвастовство',
      'Посвяти 70% эссе реакции и росту, а не самой проблеме',
      'Покажи зрелость в размышлениях об опыте',
      'Будь честен о том, что было тяжело, не изображая жертву',
    ],
    donts: [
      'Писать о трудности, которая на самом деле — замаскированное достижение',
      'Обвинять других или представлять себя полностью безупречным',
      'Выбирать слишком травматичную тему, если не можешь рефлексировать о ней с дистанции',
      'Заканчивать аккуратным, тривиальным уроком, который звучит натянуто',
    ],
  },
}
...
function ActivityDescriptions() {
  const { t, locale } = useI18n()
  const examples = locale === 'ru' ? activityExamplesRu : activityExamples
  return (
...
function HonorsAwards() {
  const { t, locale } = useI18n()
  const honors = locale === 'ru' ? honorExamplesRu : honorExamples
  const notHonorsList = locale === 'ru' ? notHonorsRu : notHonors
  return (
...
function ResearchProjects() {
  const { t, locale } = useI18n()
  const projects = locale === 'ru' ? researchExamplesRu : researchExamples
  return (
    <div className="space-y-6">
      <div className="mb-2">
...
function PersonalStatementSection() {
  const { t, locale } = useI18n()
  const data = locale === 'ru' ? essayApproachesRu.personal : essayApproaches.personal
  return (
...
function WhyUsSection() {
  const { t, locale } = useI18n()
  const data = locale === 'ru' ? essayApproachesRu.whyUs : essayApproaches.whyUs
  return (
    <div className="space-y-5">
      <Card>
...
function CommunitySection() {
  const { t, locale } = useI18n()
  const data = locale === 'ru' ? essayApproachesRu.community : essayApproaches.community
  return (
    <div className="space-y-5">
      <Card>
...
function ChallengeSection() {
  const { t, locale } = useI18n()
  const data = locale === 'ru' ? essayApproachesRu.challenge : essayApproaches.challenge
  return (
    <div className="space-y-5">
      <Card>
